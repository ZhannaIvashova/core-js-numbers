...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthSecongtVector = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = dotProduct / (lengthFirstVector * lengthSecongtVector);
...
function roundToPowerOfTen(num, pow) {
  ...
  const division = num / toPower;
  return Math.round(division) * toPower;
}
